...
def discretized_logistic_logp(mu, scale, x):
    ...
    plus_in = invscale * (x_centered + 1. / 511.)
    cdf_plus = torch.sigmoid(plus_in)
    # min_in = invscale * (x_centered - 1. / 255.)
    min_in = invscale * (x_centered - 1. / 511.)
...
# can be train for codes, loss is around 5.0, but atually compression doesnt help
# def discretized_logistic_logp(mu, scale, x):
#     # [0,255] -> [-1.1] (this means bin sizes of 2./255.)
#     x_rescaled = (x - 127.5) / 127.5
#     # [0,511] -> [-1.1] (this means bin sizes of 2./511.)
...
class ImageBins:
    ...
    def endpoints(self):
        ...
        endpoints = torch.arange(1, 512, dtype=self.type, device=self.device)
        endpoints = ((endpoints - 255.5) / 255.5) - 1. / 511.
    ...
    def centres(self):
        ...
        centres = torch.arange(0, 512, dtype=self.type, device=self.device)
        centres = (centres - 255.5) / 255.5
        centres = centres[None,].expand(self.shape + [-1])
        return centres

# ImageBins for image will casue error for codes ' CUDA error: device-side assert triggered'
#
